...
| 图片理解与出处核验 | Rolebot 0.3.1 调用 Qwen3.8-Max | 多图编号、引用图、24 小时缓存；需要时文字搜索；DeepSeek 负责人格回复 |
| 按需联网、时间 | Rolebot 0.3.1 | 使用现有百炼密钥检索，返回实际来源；当前时间按北京时间 |
...
`prob 0` 只关闭随机参与；`off` 暂停本群回复与复读，`on` 重新开启并解除静默。`clear` 新建本群短期会话并清空近期群消息缓存，其他群和私聊不受影响。群内禁用 AstrBot 内置管理命令，保留 Rolebot 的受权限限制的管理入口。群聊不用私聊长期记忆，低频主动聊天仍仅发送给原先指定的好友。
