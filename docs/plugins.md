...
| 图片理解与出处核验 | Rolebot 0.1.1 调用 Qwen3.8-Max | 多图编号、引用图、24 小时缓存；需要时文字搜索；DeepSeek 负责人格回复 |
| 按需联网、时间 | Rolebot 0.1.1 | 使用现有百炼密钥检索，返回实际来源；当前时间按北京时间 |
...
通用识图按本轮问题提取场景、物品、文字、数值、截图细节或角色信息，并把与问题有关的结论交给聊天模型；无需预先给每张图配置角色答案。原图进入搜索复核，搜索综合摘要与网页来源分别标注，查不到作者不会自动否定图中角色。当前 AstrBot 的 OpenAI 适配器丢弃普通请求参数，插件通过请求内独立配置传递 JSON Schema；格式不支持时在同一个超时预算内降级，不修改共享提供商设置。
